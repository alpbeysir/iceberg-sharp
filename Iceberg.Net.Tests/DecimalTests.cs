...
public class DecimalTests
{
    ...
    {
        Decimal128Type arrowType = DecimalRow.ArrowSchema
            .GetFieldByName(nameof(DecimalRow.Wide))
            .DataType
            .Should()
            .BeOfType<Decimal128Type>()
            .Subject;
        arrowType.Precision.Should().Be(38);
        arrowType.Scale.Should().Be(18);

        Schema icebergSchema = CSharpSchemas.ToIcebergSchema(typeof(DecimalRow), null, _ => 1);
        icebergSchema.Fields
            .Single(field => field.Name == nameof(DecimalRow.Wide))
            .FieldType
            .Should()
            .Be(new PrimitiveType.Decimal(38, 18));
        icebergSchema.Fields
            .Single(field => field.Name == nameof(DecimalRow.Compact))
            .FieldType
            .Should()
            .Be(new PrimitiveType.Decimal(9, 2));
    }
    ...
    public void SqlDecimalRoundTripsWithoutLosingPrecision()
    {
        ...
        actual.Select(row => row.Wide?.ToString())
            .Should()
            .Equal(expected.Select(row => row.Wide?.ToString()));
        actual.Select(row => row.Compact.ToString())
            .Should()
    ...
    public void ClrDecimalIsNotSupported()
    {
        ...
        Action buildIceberg = () => CSharpSchemas.ToIcebergSchema(typeof(ClrDecimalRow), null, _ => 1);
...
{
    [DecimalWith(38, 18)] public required SqlDecimal? Wide { get; init; }

    [DecimalWith(9, 2)] public required SqlDecimal Compact { get; init; }
