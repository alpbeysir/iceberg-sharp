...
/// </summary>
public sealed class OcfReaderAsync : IAsyncDisposable
{
    ...
    private readonly AvroSchema _writerAvroSchema;
    ...
    internal AvroRecordSchema WriterRecordSchema => _writerSchema;
    public AvroSchema WriterSchema => _writerAvroSchema;
    public AvroCodec Codec => _codec;
    public IReadOnlyDictionary<string, byte[]> Metadata => _metadata;

    private OcfReaderAsync(PipeReader pipeReader, AvroRecordSchema writerSchema, AvroSchema writerAvroSchema,
        AvroCodec codec,
        byte[] syncMarker, IReadOnlyDictionary<string, byte[]> metadata)
    {
        _pipeReader = pipeReader;
        _writerSchema = writerSchema;
        _writerAvroSchema = writerAvroSchema;
        _codec = codec;
    ...
    public static async ValueTask<OcfReaderAsync> OpenAsync(Stream stream, CancellationToken ct = default)
    {
        ...
        var writerSchema = new AvroSchema(schemaJson);
        var schemaNode = writerSchema.Parsed;
        ...
        return new OcfReaderAsync(pipeReader, recordSchema, writerSchema, codec, syncMarker, metadata);
    }
