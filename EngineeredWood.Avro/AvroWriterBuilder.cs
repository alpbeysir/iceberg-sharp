...
public sealed class AvroWriterBuilder
{
    ...
    private IReadOnlyDictionary<string, byte[]>? _metadata;
    ...
    /// <summary>Add arbitrary application metadata to the OCF header.</summary>
    public AvroWriterBuilder WithMetadata(IReadOnlyDictionary<string, byte[]> metadata)
    {
    ...
    public AvroWriter Build(Stream output)
    {
        ...
        return new AvroWriter(ocf, _arrowSchema, avroRecord, avroSchema, _metadata);
    }
    ...
    public async ValueTask<AvroAsyncWriter> BuildAsync(Stream output, CancellationToken ct = default)
    {
        ...
        await ocf.WriteHeaderAsync(avroSchema, _metadata, ct).ConfigureAwait(false);
        return new AvroAsyncWriter(ocf, _arrowSchema, avroRecord, avroSchema);
    }
