...
public readonly record struct ManifestEntry
{
    ...
    {
        var entryReader = new ManifestEntryV2Reader(AvroSchema, AvroSchema);
        var fileReader = DataFileReader<ManifestEntry>.OpenReader(
            stream,
            AvroSchema,
            (_, _) => entryReader);
        ...
        var partitionSpecReader = new GenericReader<GenericRecord>(
    ...
    public static IFileWriter<ManifestEntry> GetAppender(
        ...
        Content content)
    {
        var datumWriter = new ManifestEntryV2Writer
        {
            PartitionSpecWriter = GetPartitionSpecWriter(partitionSpec)
        };
        var writer = DataFileWriter<ManifestEntry>.OpenWriter(datumWriter, stream, true);
    ...
    private static Action<GenericRecord, Encoder> GetPartitionSpecWriter(PartitionSpec partitionSpec)
    {
        ...
        var partitionSpecWriter = new DefaultWriter(Utils.EmptyPartitionAvroSchema);
    ...
    private record ManifestEntryV2Reader(Avro.Schema ReaderSchema, Avro.Schema WriterSchema)
